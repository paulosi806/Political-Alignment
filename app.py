...
# ── Speaker data preparation (cached per congress selection) ───────────────────

@st.cache_data(show_spinner="Preparing speaker data…")
def prepare_speaker_data(
    ...
    speaker_data = ss[meta_cols].drop_duplicates().copy()
...
with st.sidebar:
    ...
    selected_congresses = st.multiselect(
        "Congress(es) to include",
        options=all_congresses,
        default=[119],
        format_func=lambda c: f"{c} ({CONGRESS_YEARS.get(c, '')})",
        help="Fewer congresses = much faster. Start with one.",
    )
    ...
    max_phrases = st.number_input(
        ...
        value=100,
        step=25,
        help="Top N most-frequent phrases per Congress. Lower = faster.",
    )
    ...
    lambda_steps = st.slider(
        ...
        max_value=100,
        value=10,
        help="Regularization path length. Lower = faster, less precise.",
    )

    top_n = st.slider("Top phrases to show", min_value=5, max_value=50, value=15)
    top_n_dir = st.slider("Top phrases per direction", min_value=5, max_value=25, value=10)

    st.divider()
    st.caption(
        "💡 Recommended starting point: 1 Congress, 100 phrases, 10 lambda steps. "
        "Expect ~1–3 min on the free Streamlit Cloud tier."
    )
...
if not selected_congresses:
    st.warning("Select at least one Congress in the sidebar.")
    st.stop()
...
with info_col:
    st.caption(
        f"Congress: **{congress_labels}** · "
        f"up to {int(max_phrases):,} phrases · "
...
if "model_result" in st.session_state:
    ...
    st.caption(
        ...
        "The higher the value, the more a classifier can correctly identify party from speech."
    )
    ...
    selected_result_congress = st.selectbox(
        "Congress",
        result_congresses,
        format_func=lambda c: f"{c} ({CONGRESS_YEARS.get(c, '')})",
    )

    phrase_probs_t = phrase_probs[phrase_probs["congress"] == selected_result_congress].copy()
    ...
    with tab_direction:
        ...
        with dir_col1:
            ...
            st.dataframe(rep_phrases[dir_show_cols], use_container_width=True, hide_index=True)
        with dir_col2:
            st.markdown("**🔵 Democratic phrases**")
            st.dataframe(dem_phrases[dir_show_cols], use_container_width=True, hide_index=True)
